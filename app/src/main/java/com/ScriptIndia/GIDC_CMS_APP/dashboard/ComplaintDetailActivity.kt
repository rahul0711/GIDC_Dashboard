...
class ComplaintDetailActivity : AppCompatActivity() {
    ...
    private var selectedBeforeImageUri: Uri? = null
    private var beforeCameraImageUri: Uri? = null
    ...
    private val takeBeforePhoto =
        registerForActivityResult(ActivityResultContracts.TakePicture()) { success ->
            if (success) {
                selectedBeforeImageUri = beforeCameraImageUri
            } else {
                if (selectedBeforeImageUri == beforeCameraImageUri) selectedBeforeImageUri = null
            }
...
//        val tvName = findViewById<TextView>(R.id.tvEngResolvedPhotoName) ?: return
    ...
    private fun bindAgencyView(item: ComplaintModel, isAlertOrResolve: Boolean) {
        ...
        fun shouldShowBeforeImage(selected: String): Boolean {
            return selected.trim().equals("in process", ignoreCase = true)
        }

        fun refreshImageVisibility() {
            val selectedStatus = spinner?.selectedItem?.toString() ?: ""
            val require = shouldRequireImage(selectedStatus)
            val showBeforeImage = shouldShowBeforeImage(selectedStatus)
            rowResolvedImage?.visibility = if (require) View.VISIBLE else View.GONE
            rowBeforeImage?.visibility = if (showBeforeImage) View.VISIBLE else View.GONE
            if (!require) {
                selectedResolvedImageUri = null
                updateSelectedImageUi()
            }
            if (!showBeforeImage) {
                selectedBeforeImageUri = null
                updateSelectedBeforeImageUi()
            }
        }
        ...
        btnUpdate?.setOnClickListener {
            ...
            updateComplaintMultipart(
                item = item,
                agencyId = agencyId,
                status = selectedStatus,
                notes = notes,
                imageUri = selectedResolvedImageUri,
                beforeImageUri = selectedBeforeImageUri
            )
        }
    ...
    private fun updateComplaintMultipart(
        ...
        val beforeImagePart: MultipartBody.Part? = try {
            beforeImageUri?.let { uri ->
                val tmp = copyUriToTempFile(contentResolver, uri, "before_${item.complainFormID}")
                val mediaType = contentResolver.getType(uri)?.toMediaType() ?: "image/*".toMediaType()
                val body = tmp.asRequestBody(mediaType)
                MultipartBody.Part.createFormData("ComplainBeforeImage", tmp.name, body)
            }
        } catch (e: Exception) {
            Log.e(TAG, "Before image prepare failed: ${e.message}", e)
            null
        }
        ...
        lifecycleScope.launch {
            try {
                val response = RetrofitClient.api.updateComplainMultipart(
                    ...
                    ComplainBeforeImage = beforeImagePart
                )
    ...
    private fun updateSelectedImageUi() {
        ...
        bindSelectedImageName(tv, selectedResolvedImageUri)
    }

    private fun updateSelectedBeforeImageUi() {
        val tv = findViewById<TextView>(R.id.tvAgcSelectedBeforeImage) ?: return
        bindSelectedImageName(tv, selectedBeforeImageUri)
    }

    private fun bindSelectedImageName(tv: TextView, uri: Uri?) {
        if (uri == null) {
    ...
    private fun launchBeforeCameraCapture() {
        try {
            val imageFile = File(cacheDir, "before_camera_${System.currentTimeMillis()}.jpg")
            val uri = FileProvider.getUriForFile(
                this,
                "${applicationContext.packageName}.fileprovider",
                imageFile
            )
            beforeCameraImageUri = uri
            takeBeforePhoto.launch(uri)
        } catch (e: Exception) {
            Log.e(TAG, "Before camera launch failed: ${e.message}", e)
            Toast.makeText(this, "Unable to open camera: ${e.message}", Toast.LENGTH_SHORT).show()
        }
    }
